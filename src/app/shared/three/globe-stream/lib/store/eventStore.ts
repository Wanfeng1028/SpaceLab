...
class EventStore {
  ...
  registerBuildInEventMap(eventName: string, cb: () => void) {
    const handler = ((event: MouseEvent) => {
      ...
      cb();
      if (eventMesh && eventMesh.userData['type'] === 'country' && this.areaColorNeedChange) {
  ...
  handleRaycaster(event: MouseEvent) {
    ...
    const intersects = this.raycaster.intersectObjects(this._chartScene.scene.children, true);
