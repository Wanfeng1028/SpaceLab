...
import CustomOrbitControls from './utils/controls';
...
export default class ChartScene {
   ...
   */
  on(eventName: string, cb: (event: Event, mesh: Object3D | Group | Mesh | undefined) => void) {
  ...
  addFigures3d() {
    ...
    // 如果是混合纹理或者不使用纹理
    if (this.options.config.texture?.mixed || !this.options.config.texture?.path) {
  ...
  createRender(): WebGLRenderer {
    ...
    if (this.options.dom instanceof HTMLCanvasElement) {
      canvas = this.options.dom;
    }
   ...
   */
  createCountryNamesText(): Group | null {
    const textConfig = this.options.config.textMark;
    if (!textConfig?.data?.length) {
      return null;
    }
    ...
    textConfig.data.forEach((item) => {
      const textGroup = textMark.create(item);
      group.add(textGroup);
    });
