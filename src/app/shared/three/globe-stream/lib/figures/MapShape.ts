...
export default class MapShape {
  ...
  create2d(countryCoordinates: Position[][][]) {
    ...
    countryCoordinates.forEach((subItem: Position[][]) => {
      if (!subItem[0]) return;
      const { linePoints2d, allPoints2d, usefulIndexArr } = this.gridPoint(subItem[0]);
      const shapeGeometry = this.createShapeGeometry(usefulIndexArr, allPoints2d);
  ...
  create3d(countryCoordinates: Position[][][]) {
    ...
    countryCoordinates.forEach((subItem: Position[][]) => {
      if (!subItem[0]) return;
      const { linePoints3d, allPoints3d, usefulIndexArr } = this.gridPoint(subItem[0]);
      const shapeGeometry = this.createShapeGeometry(usefulIndexArr, allPoints3d);
      this.geometryArr.push(shapeGeometry);
  ...
  createMaterial() {
    const materialType: BasicMaterial =
      this.currentStyle.material || this._config.mapStyle.material || 'MeshBasicMaterial';
    const transparent =
      typeof this.currentStyle.opacity === 'number' && this.currentStyle.opacity < 1;
    const baseConfig = {
