...
export default class Scatter {
  ...
  setMeshAttr(
    ...
    const size = this._currentStyle.size || this._config.scatterStyle.size || this._config.R * 0.05;
  ...
  createCustomMesh = (data: Coordinates) => {
    ...
    if (this._currentStyle.customFigure?.rotate) {
      mesh.rotation.z = this._currentStyle.customFigure.rotate * (Math.PI / 180);
    }
  ...
  createCustom() {
    const geometry = new PlaneGeometry(1, 1);
    const textureLoader = new TextureLoader().load(this._currentStyle.customFigure?.texture!);
    const material = new MeshBasicMaterial({
      map: textureLoader,
      transparent: true,
      color: this._currentStyle.color,
  ...
  create(data: Coordinates) {
    ...
    const show = this._currentStyle.show ?? this._config.scatterStyle.show ?? true;
