...
export default class FlyLine3d {
  ...
  createMesh(positionInfo: [Vector3, Vector3]) {
    ...
    const middlePos = dir.multiplyScalar(this._config.R + s * this._config.R * flyLineRFactor);
    //寻找三个圆心的坐标
  ...
  createImg(R: number, startAngle: number, endAngle: number) {
    ...
    const group = new Group();
    // 创建纹理加载器
    const texture = this.textureLoader.load(this._currentConfig.flyLineStyle.img!);
    // 创建精灵材质，使用加载的纹理
    const { color, opacity } = parseColorOpacity(this._currentConfig.flyLineStyle.color);
  ...
  createPathLine = (middlePos: Vector3, r: number, startDeg: number, endDeg: number) => {
    ...
    const { color, opacity } = parseColorOpacity(this._currentConfig.pathStyle.color);
