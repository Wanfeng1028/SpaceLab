...
function isFeatureCollection(source: unknown): source is FeatureCollection<Geometry> {
  return (
    !!source &&
...
function appendMultiPolygonRings(multiPolygon: MultiPolygon['coordinates'], lines: Position[][]) {
  multiPolygon.forEach((polygon) => appendPolygonRings(polygon, lines));
}

function appendGeometryCollection(collection: GeometryCollection, lines: Position[][]) {
...
function appendGeometry(geometry: Geometry | null, lines: Position[][]) {
  ...
  switch (geometry.type) {
    ...
    case 'MultiLineString':
      (geometry as MultiLineString).coordinates.forEach((line) => appendLine(line, lines));
      return;
    case 'Polygon':
