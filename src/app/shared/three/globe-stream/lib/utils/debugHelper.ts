...
export default class DebugHelper {
  ...
  constructor(chart: ChartScene) {
    ...
    this.buildControls();
    const mountTarget = this.chart.options.dom.parentElement || this.chart.options.dom;
  ...
  buildControls() {
    ...
    const defaultRotateSpeed =
      this.chart.initOptions.rotateSpeed !== undefined ? this.chart.initOptions.rotateSpeed : 0.01;
  ...
  addControl(control: ControlConfig) {
    ...
    if (control.type === 'select') {
      ...
      if (control.type === 'color') {
        ...
        const numericValue =
          typeof control.value === 'number' ? control.value : Number(control.value);
  ...
  updateCountryAreaColor(color: string) {
    this.chart.scene.traverse((obj: Object3D) => {
  ...
  updateCountryMaterial(materialKey: BasicMaterial) {
    ...
    this.chart.scene.traverse((obj: Object3D) => {
      if (obj instanceof Mesh && obj.userData['type'] === 'country') {
        ...
        const material = this.createCountryMaterial(materialKey, color, opacity);
        if (material) {
  ...
  private createCountryMaterial(materialKey: BasicMaterial, color: Color, opacity: number) {
  ...
  updateSpriteSize(size: number) {
    this.chart.scene.traverse((obj: Object3D) => {
      if (obj instanceof Sprite) {
        obj.scale.set(
          this.chart._store.getConfig().R * size,
          this.chart._store.getConfig().R * size,
          1,
        );
      }
  ...
  updateScatterColor(color: string) {
    this.chart.scene.traverse((obj: Object3D) => {
      if (obj instanceof Mesh && (obj.name === 'scatter' || obj.name === 'point')) {
        const material = obj.material as any;
        if (material && material.color) {
          material.color.set(color);
          material.needsUpdate = true;
