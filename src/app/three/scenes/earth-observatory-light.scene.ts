...
export class EarthObservatoryLightScene {
  ...
  private createNoiseTexture(): DataTexture {
    const size = 256;
    ...
    for (let y = 0; y < size; y++) {
      for (let x = 0; x < size; x++) {
        ...
        data[y * size + x] = Math.floor(
          (Math.sin(nx * 12.9898 + ny * 78.233) * 43758.5453 % 1) * 255
        );
      }
    }
    ...
    const mat = new ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          // Terrain pattern using pre-baked noise texture
          float terrain = texture2D(uNoiseTex, vUv * 8.0 + vec2(uTime * 0.1)).r;
          float continent = texture2D(uNoiseTex, vUv * 3.0).r;
      ...
      `,
      uniforms: { uTime: { value: 0 }, uNoiseTex: { value: this.noiseTexture } },
    });
